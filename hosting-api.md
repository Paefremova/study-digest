...
Токены: строками `GITVERSE_TOKEN` и `SOURCECRAFT_TOKEN` в `config.env` (права 600; запасной
вариант — файлы `~/.config/<сервис>/token`). Репозиторий определяется по git-remote того каталога, откуда запущена команда,
