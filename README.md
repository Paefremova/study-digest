...
отбрасываются по `eventtype`), «Новое в курсах», «Тесты» (за месяц до теста; пройденные — с попыткой `finished` — не показываются),
...
учитывается, только если его адрес ведёт на соответствующий хостинг, иначе берётся
`GV_REPO` / `SC_REPO` из `config.env`. `gv release`
...
по срокам, но локального состояния (git/релизы/лабы) у него нет.

Если по календарю срок есть у курса вне списка — в игноре или скрытого в ТУИС, — сводка
сообщит об этом отдельным разделом.
...
команда запускается повторно: незаполненные поля в текст не попадают, список
недостающих печатается в конце. Готовый текст
