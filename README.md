...
**ДЗ 2 — Нормализация до 3НФ** · databases · до 06.10 23:59 · методички: databases/stash/
```
...
вывод как есть — сводка уже свёрстана программой. От агента остаётся раздел «Предлагаю
начать»: по методичке самой срочной работы он пишет, что требуется и сколько займёт, и
предлагает первое сообщение для новой сессии. Промпт задачи — [`docs/daily-digest-prompt.md`](docs/daily-digest-prompt.md).
...
машиночитаемо — ровно то, что видит текстовый вывод, без второго обхода API.
...
срокам, но локального состояния (git, релизы, лабы) у него нет. Срок у курса вне списка —
в игноре или скрытого — сводка показывает отдельным разделом.

**Снимок состояния.** «Новое» в сводке — новое с прошлого запуска: `.digest/.state.json`
(время запуска, сроки, баллы) обновляется каждым `state`/`digest` без `--no-save`, копия
...
это взгляд назад, без него следующий снимок перепишется.
...
работает с токенами. `study update --check` показывает то же без изменений.
